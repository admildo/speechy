...
let screenshots
let screenWidth = 1820
let screenHight = 1080
...
function createWindow() {
  ...
  mainWindow.on('focus', () => {
    mainWindow.setOpacity(1) // Ensure full opacity on focus
    // Or apply any specific styles or classes if needed
  })

  mainWindow.on('blur', () => {
    mainWindow.setOpacity(0.9) // Adjust opacity if needed on blur
    // Or revert any styles or classes
  })
...
*/
  ...
  ipcMain.handle('force-redraw', () => {
    mainWindow.webContents.invalidate()
  })
  ipcMain.handle('resize-window', async (event, position) => {
    console.log('Resize window function called with position:', position)
    // Log each property of the position object separately for clarity
    console.log('Width:', position.width)
    console.log('Height:', position.height)
    console.log('X:', position.x)
    console.log('Y:', position.y)

    mainWindow.setSize(position.width, position.height)
    mainWindow.setPosition(position.x, position.y)
    console.log('should resize')
    return true // Return true to indicate the message was received
  })
